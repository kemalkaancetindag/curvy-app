...
class MatchService extends GetxService {
  ...
  Future<void> createMatch(String likedUserID) async {
    ...
    var matchCheckSnapshot = await firestoreService
    ...
    var matchCheckSnapshotReverse = await firestoreService
        .getCollection('matches')
    ...
    if (matchCheckSnapshot.docs.isEmpty &&
        matchCheckSnapshotReverse.docs.isEmpty) {
      var currentUserDoc = (await firestoreService
              .getCollection('users')
              .where('userID', isEqualTo: currentUserID)
              .get())
          .docs[0];
      var likedUserDoc = (await firestoreService
              .getCollection('users')
      ...
      var likedUserNewUsersWhoLikedMeList =
          UserModel.fromJson(likedUserDoc.data() as Map<String, dynamic>)
              .users_who_liked_me;
      ...
      if (likedUserNewUsersWhoLikedMeList == null) {
        ...
        likedUserUpdateData['users_who_liked_me'] =
            likedUserNewUsersWhoLikedMeList;
      }

      var currentUsersNewUsersILikedList =
      ...
      await firestoreService
          ...
          .update(currentUserUpdateData);
      await firestoreService
          .getCollection('users')
          .doc(likedUserDoc.id)
          .update(likedUserUpdateData);
      ...
      if (matchCheckSnapshot.docs.isNotEmpty) {
        var currentUserDoc = (await firestoreService
                .getCollection('users')
                .where('userID', isEqualTo: currentUserID)
                .get())
        ...
        await firestoreService
            ...
            .doc(likedUserDoc.id)
            .update(likedUserUpdateData);
        firestoreService.updateMatch(currentUserID, likedUserID);
      } else if (matchCheckSnapshotReverse.docs.isNotEmpty) {
                var currentUserDoc = (await firestoreService
                .getCollection('users')
                .where('userID', isEqualTo: currentUserID)
                .get())
        ...
        await firestoreService
            ...
            .doc(likedUserDoc.id)
            .update(likedUserUpdateData);
        firestoreService.updateMatch(likedUserID, currentUserID);
      }
    }
  }
  ...
  Future<void> dislikeUser(String dislikedUserID) async {
    ...
    var currentUserDoc = (await firestoreService
            .getCollection('users')
            .where('userID', isEqualTo: currenUserID)
            .get())
        .docs[0];
    var user =
        UserModel.fromJson(currentUserDoc.data() as Map<String, dynamic>);
  ...
  Future<void> removeLastAction(String interractedUserID) async {
    ...
    var interractedUserDoc = (await firestoreService
            .getCollection('users')
            .where('userID', isEqualTo: interractedUserID)
            .get())
        .docs[0];
    var interractedUser =
        UserModel.fromJson(interractedUserDoc.data() as Map<String, dynamic>);
    ...
    if (currentUser.remaining_daily_back_count! > 0) {
      var matches = (await firestoreService
              .getCollection('matches')
              .where('user1.id', isEqualTo: currentUserID)
              .where('user2.id', isEqualTo: interractedUserID)
              .get())
          .docs;
      ...
      if (matches.isNotEmpty) {
        ...
        var newCurrentUserLikedList = currentUser.users_i_liked!
            .where((id) => id != interractedUserID)
            .toList();
        var newInterractedUserWhoLikedList = interractedUser.users_who_liked_me!
            .where((id) => id != currentUserID)
            .toList();

        currentUserUpdateData['users_i_liked'] = newCurrentUserLikedList;
        interractedUserUpdateData['users_i_liked'] =
            newInterractedUserWhoLikedList;

        await firestoreService.updateUser(
            currentUserUpdateData, currentUserID!);
        await firestoreService.updateUser(
            interractedUserUpdateData, interractedUserID);
        ...
        var newCurrentUserUnlikedList = currentUser.un_liked_users!
            .where((id) => id != interractedUserID)
            .toList();
